...
% Dedicated title above axis (prevents overlap/clipping)
...
%% ==========================================================
% RIGHT PANEL (CUSTOM BLACK TABS, NO WHITE BOXES)
%% ==========================================================
...
% Status/info line (always visible)
...
%% ---- Bottom right buttons (outside controls panel) ----
...
function layoutUI()
    ...
    set(info1,'Position',[contentX 10 contentW statusH]);
...
%% ===================== ROI EXPORT (FIXED INDEXING + SUBFOLDERS) =====================
function exportROIsCB(~,~)
    ...
    try
        ...
        % --- FIX: start from next available roi index (no overwrites)
        ...
        for zz=1:nZ
            ...
            for k=1:numel(ROI)
                ...
                % ultra-safe: in case file exists, bump until free
...
function roiDir = getAutoRoiDir()
    ...
    % --- FIX: folder split by dataset type (raw/gabriel/...)
    dsType = deriveDatasetType();      % e.g. 'raw' or 'gabriel'
    tag0   = deriveDatasetTag();       % sanitized dataset tag
...
function dsType = deriveDatasetType()
    % prefer explicit studio info if available; else fall back to fileLabel
    s = '';
    try
        if isstruct(par)
    ...
    % --- IMPORTANT: if s is a path, keep only the filename (prevents "RawData" triggering "raw")
try
    ss = char(s);
    % handle .nii.gz
    ...
    s = lower(strtrim(s));
    s = regexprep(s,'\|.*$','');   % drop anything after |
    s = regexprep(s,'\(.*$','');   % drop (...) tail
    s = strtrim(s);

    if contains(s,'gabriel'), dsType = 'gabriel'; return; end
    % Only match "raw" as a token, not as part of folder names like "RawData"
if ~isempty(regexp(s,'(^|[_\-\s])raw([_\-\s]|$)','once'))
    dsType = 'raw'; return;
end


    % try first token as type
