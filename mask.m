...
%       mask / loadedMask = overlayMask
%
% This is done on purpose so existing play_fusi_video_final-style code
% that loads "mask" or "loadedMask" will automatically use the overlay
% restriction mask for PSC / alpha-modulated display.
%
% SAVED VARIABLES
%   brainImage
%   brainMask
...
% 2) Theme
% =========================================================
C.bg0   = [0.05 0.05 0.06];
C.bg1   = [0.10 0.10 0.12];
C.bg2   = [0.14 0.14 0.17];
C.fg    = [0.95 0.95 0.95];
C.sub   = [0.74 0.88 1.00];

C.ok    = [0.22 0.75 0.45];
C.bad   = [0.88 0.24 0.24];
C.blue  = [0.25 0.55 0.95];
C.warn  = [0.95 0.60 0.15];
C.dim   = [0.30 0.30 0.34];

C.brain = [0.15 0.85 0.32];
C.ovl   = [0.88 0.20 0.70];
C.red   = [0.95 0.25 0.25];
C.gold  = [1.00 0.85 0.15];
...
S.smoothSize = 8;
S.brushR = 18;
S.brushShape = 1; % 1 round, 2 square, 3 pen, 4 diamond
...
% 7) Helper maker
% =========================================================
    function h = makeText(parent, pos, str, col, fs, fw, ha)
        if nargin < 7 || isempty(ha), ha = 'left'; end
        if nargin < 6 || isempty(fw), fw = 'normal'; end
        if nargin < 5 || isempty(fs), fs = 12; end
        if nargin < 4 || isempty(col), col = C.fg; end
        h = uicontrol('Style','text','Parent',parent,'Units','normalized', ...
            'Position',pos, ...
            'String',str, ...
            'BackgroundColor',C.bg1, ...
            'ForegroundColor',col, ...
...
h.togPreview = uicontrol('Style','togglebutton','Parent',pMode,'Units','normalized', ...
    'Position',[0.52 0.72 0.46 0.18], ...
    'String','Preview: FULL','Value',0, ...
    'BackgroundColor',C.blue,'ForegroundColor','w', ...
    'FontSize',14,'FontWeight','bold', ...
    'Callback',@onTogglePreview);

h.btnTargetBrain = uicontrol('Style','pushbutton','Parent',pMode,'Units','normalized', ...
    'Position',[0.02 0.46 0.46 0.16], ...
    'String','UNDERLAY / BRAIN', ...
    'BackgroundColor',C.brain,'ForegroundColor','w', ...
    'FontSize',13,'FontWeight','bold', ...
    'Callback',@onTargetBrain);

h.btnTargetOverlay = uicontrol('Style','pushbutton','Parent',pMode,'Units','normalized', ...
    'Position',[0.52 0.46 0.46 0.16], ...
    'String','OVERLAY / SIGNAL', ...
    'BackgroundColor',[0.28 0.18 0.30],'ForegroundColor',[0.90 0.90 0.90], ...
    'FontSize',13,'FontWeight','bold', ...
    'Callback',@onTargetOverlay);

h.txtTargetInfo = makeText(pMode,[0.03 0.28 0.94 0.10],'Active: Underlay / Brain mask',C.brain,13,'bold','left');

h.chkShowOverlay = uicontrol('Style','checkbox','Parent',pMode,'Units','normalized', ...
    'Position',[0.03 0.13 0.45 0.10], ...
    'String','Show overlay', ...
...
h.lblOverlayAlpha = makeText(pMode,[0.03 0.02 0.12 0.10],'Alpha',C.fg,12,'normal','left');
h.slOverlayAlpha = uicontrol('Style','slider','Parent',pMode,'Units','normalized', ...
    'Position',[0.16 0.05 0.61 0.06], ...
    'Min',0,'Max',1,'Value',S.overlayAlpha, ...
    'Callback',@onOverlayAlphaChange);
h.txtOverlayAlpha = makeText(pMode,[0.80 0.02 0.15 0.10],sprintf('%.2f',S.overlayAlpha),C.fg,12,'normal','right');

% Save panel
h.btnSaveBrain = uicontrol('Style','pushbutton','Parent',pSave,'Units','normalized', ...
    'Position',[0.00 0.16 0.31 0.60], ...
    'String','SAVE BRAIN', ...
    'BackgroundColor',C.ok,'ForegroundColor','w', ...
    'FontSize',13,'FontWeight','bold', ...
    'Callback',@onSaveBrain);

h.btnSaveOverlay = uicontrol('Style','pushbutton','Parent',pSave,'Units','normalized', ...
    'Position',[0.345 0.16 0.31 0.60], ...
    'String','SAVE OVERLAY', ...
    'BackgroundColor',C.ovl,'ForegroundColor','w', ...
    'FontSize',13,'FontWeight','bold', ...
    'Callback',@onSaveOverlay);

h.btnSaveBoth = uicontrol('Style','pushbutton','Parent',pSave,'Units','normalized', ...
    'Position',[0.69 0.16 0.31 0.60], ...
    'String','SAVE BOTH', ...
    'BackgroundColor',C.blue,'ForegroundColor','w', ...
    'FontSize',13,'FontWeight','bold', ...
    'Callback',@onSaveBoth);
...
h.btnLoadUnderlay = uicontrol('Style','pushbutton','Parent',pUnder,'Units','normalized', ...
    'Position',[0.03 0.59 0.94 0.12], ...
...
h.txtUnderlayLabel = makeText(pUnder,[0.03 0.32 0.94 0.10],['Underlay: ' UbaseLabel],[0.70 0.85 1.00],11,'normal','left');

h.lblDbLow = makeText(pUnder,[0.03 0.17 0.15 0.10],'dB low',C.fg,11,'normal','left');
h.edDbLow = uicontrol('Style','edit','Parent',pUnder,'Units','normalized', ...
    'Position',[0.18 0.18 0.18 0.12], ...
    'String',num2str(S.dbLow), ...
    'BackgroundColor',C.bg2,'ForegroundColor','w', ...
    'FontSize',11, ...
    'Callback',@onDbEdit);

h.lblDbHigh = makeText(pUnder,[0.43 0.17 0.15 0.10],'dB high',C.fg,11,'normal','left');
h.edDbHigh = uicontrol('Style','edit','Parent',pUnder,'Units','normalized', ...
    'Position',[0.58 0.18 0.18 0.12], ...
...
if nZ > 1
    h.slSlice = uicontrol('Style','slider','Parent',pUnder,'Units','normalized', ...
        'Position',[0.03 0.03 0.72 0.08], ...
        'Min',1,'Max',nZ,'Value',S.z, ...
        'SliderStep',[1/max(1,nZ-1) 5/max(1,nZ-1)], ...
        'Callback',@onSliceChange);
    h.txtSliceVal = makeText(pUnder,[0.76 0.02 0.20 0.10],sprintf('z=%d/%d',S.z,nZ),C.fg,11,'normal','right');
else
    h.slSlice = [];
    h.txtSliceVal = [];
end
...
% Display panel
h.lblBright = makeText(pDisplay,[0.03 0.79 0.18 0.10],'Bright',C.fg,11,'normal','left');
h.slBright = uicontrol('Style','slider','Parent',pDisplay,'Units','normalized', ...
    'Position',[0.22 0.82 0.58 0.06], ...
    'Min',-0.6,'Max',0.6,'Value',S.brightness, ...
    'Callback',@onDisplayChange);
h.txtBright = makeText(pDisplay,[0.82 0.79 0.15 0.10],sprintf('%.2f',S.brightness),C.fg,11,'normal','right');

h.lblCont = makeText(pDisplay,[0.03 0.57 0.18 0.10],'Contrast',C.fg,11,'normal','left');
h.slCont = uicontrol('Style','slider','Parent',pDisplay,'Units','normalized', ...
    'Position',[0.22 0.60 0.58 0.06], ...
    'Min',0.5,'Max',3.0,'Value',S.contrast, ...
    'Callback',@onDisplayChange);
h.txtCont = makeText(pDisplay,[0.82 0.57 0.15 0.10],sprintf('%.2f',S.contrast),C.fg,11,'normal','right');

h.lblGamma = makeText(pDisplay,[0.03 0.35 0.18 0.10],'Gamma',C.fg,11,'normal','left');
h.slGamma = uicontrol('Style','slider','Parent',pDisplay,'Units','normalized', ...
    'Position',[0.22 0.38 0.58 0.06], ...
    'Min',0.2,'Max',3.0,'Value',S.gamma, ...
    'Callback',@onDisplayChange);
h.txtGamma = makeText(pDisplay,[0.82 0.35 0.15 0.10],sprintf('%.2f',S.gamma),C.fg,11,'normal','right');

h.lblSharp = makeText(pDisplay,[0.03 0.13 0.18 0.10],'Sharp',C.fg,11,'normal','left');
h.slSharp = uicontrol('Style','slider','Parent',pDisplay,'Units','normalized', ...
    'Position',[0.22 0.16 0.58 0.06], ...
    'Min',0,'Max',300,'Value',S.sharpness, ...
    'Callback',@onDisplayChange);
h.txtSharp = makeText(pDisplay,[0.82 0.13 0.15 0.10],sprintf('%.2f',S.sharpness),C.fg,11,'normal','right');

h.popCmap = uicontrol('Style','popupmenu','Parent',pDisplay,'Units','normalized', ...
    'Position',[0.03 0.01 0.94 0.10], ...
...
% Tools panel
h.lblBrush = makeText(pTools,[0.03 0.70 0.16 0.12],'Brush',C.fg,11,'normal','left');
h.slBrush = uicontrol('Style','slider','Parent',pTools,'Units','normalized', ...
    'Position',[0.20 0.74 0.60 0.08], ...
    'Min',1,'Max',200,'Value',S.brushR, ...
    'Callback',@onBrushChange);
h.txtBrush = makeText(pTools,[0.82 0.70 0.15 0.12],sprintf('%.0f',S.brushR),C.fg,11,'normal','right');

h.popShape = uicontrol('Style','popupmenu','Parent',pTools,'Units','normalized', ...
    'Position',[0.03 0.51 0.94 0.13], ...
...
h.lblSmooth = makeText(pTools,[0.03 0.33 0.16 0.12],'Smooth',C.fg,11,'normal','left');
h.slSmooth = uicontrol('Style','slider','Parent',pTools,'Units','normalized', ...
    'Position',[0.20 0.37 0.60 0.08], ...
    'Min',0,'Max',100,'Value',S.smoothSize, ...
    'Callback',@onSmoothSize);
h.txtSmooth = makeText(pTools,[0.82 0.33 0.15 0.12],sprintf('%.0f',S.smoothSize),C.fg,11,'normal','right');

h.btnFillSlice = uicontrol('Style','pushbutton','Parent',pTools,'Units','normalized', ...
    'Position',[0.03 0.12 0.22 0.14], ...
    'String','Fill Slice', ...
    'BackgroundColor',C.gold,'ForegroundColor','k', ...
    'FontSize',11,'FontWeight','bold', ...
    'Callback',@onFillSlice);

h.btnFillAll = uicontrol('Style','pushbutton','Parent',pTools,'Units','normalized', ...
    'Position',[0.27 0.12 0.22 0.14], ...
    'String','Fill All', ...
    'BackgroundColor',[0.92 0.74 0.12],'ForegroundColor','k', ...
    'FontSize',11,'FontWeight','bold', ...
    'Callback',@onFillAll);

h.btnSmooth = uicontrol('Style','pushbutton','Parent',pTools,'Units','normalized', ...
    'Position',[0.51 0.12 0.22 0.14], ...
    'String','Smooth', ...
    'BackgroundColor',C.dim,'ForegroundColor','w', ...
    'FontSize',11,'FontWeight','bold', ...
    'Callback',@onSmooth);

h.btnClearSlice = uicontrol('Style','pushbutton','Parent',pTools,'Units','normalized', ...
    'Position',[0.75 0.12 0.22 0.14], ...
    'String','Clr Slice', ...
    'BackgroundColor',C.warn,'ForegroundColor','w', ...
    'FontSize',11,'FontWeight','bold', ...
    'Callback',@onClearSlice);

h.btnClearMask = uicontrol('Style','pushbutton','Parent',pTools,'Units','normalized', ...
    'Position',[0.03 0.00 0.94 0.10], ...
    'String','Clear Active Mask', ...
...
updateStatus('Ready. Left=ADD, Right=ERASE. Press F to fill current slice.');
renderNow();
...
% -------------------- General UI --------------------
    ...
    function updateTargetUI()
        if S.editTarget == 1
            ...
            set(h.btnTargetOverlay,'BackgroundColor',[0.28 0.18 0.30],'ForegroundColor',[0.90 0.90 0.90]);
            set(h.txtTargetInfo,'String','Active: Underlay / Brain mask','ForegroundColor',C.brain);
        else
            set(h.btnTargetBrain,'BackgroundColor',[0.16 0.24 0.18],'ForegroundColor',[0.90 0.90 0.90]);
            set(h.btnTargetOverlay,'BackgroundColor',C.ovl,'ForegroundColor','w');
            set(h.txtTargetInfo,'String','Active: Overlay / Signal mask','ForegroundColor',C.ovl);
        end
    end
...
% -------------------- Tool controls --------------------
    ...
    function onFillSlice(~,~)
        ...
        if S.editTarget == 1
            ...
            updateStatus('Filled brain mask in current slice.');
        else
            overlayMaskVol(:,:,z) = fillHolesAllSafe(overlayMaskVol(:,:,z));
            updateStatus('Filled overlay mask in current slice.');
        end
        renderNow();
    end
    ...
    function onFillAll(~,~)
        if S.editTarget == 1
            ...
            updateStatus('Filled brain mask in all slices.');
        else
            for zz = 1:nZ
                overlayMaskVol(:,:,zz) = fillHolesAllSafe(overlayMaskVol(:,:,zz));
            end
            updateStatus('Filled overlay mask in all slices.');
        end
        renderNow();
    end
...
% -------------------- Save --------------------
    ...
    function saveMaskBundle(mode)
        ...
        %
        % Existing play_fusi_video_final-style loading relies on:
        %   mask / loadedMask
        % So for overlay workflows we MUST make those equal to overlayMask.
        % =================================================
        switch lower(mode)
            case 'brain'
                mask = brainMask;
            ...
            otherwise
                % SAVE BOTH:
                %  - brainMask / underlayMask for processing
                %  - overlayMask / signalMask for display restriction
                %  - mask / loadedMask intentionally point to overlayMask
                mask = overlayMask;
                activeMask = overlayMask;
                loadedMask = overlayMask;
                maskIsInclude = true;
...
% -------------------- Render --------------------
    function renderNow()
        ...
        if any(Bsl(:))
            ...
            if any(Eb(:))
                e = single(Eb);
                RGB(:,:,1) = max(RGB(:,:,1), 0.12*e);
                RGB(:,:,2) = max(RGB(:,:,2), 1.00*e);
                RGB(:,:,3) = max(RGB(:,:,3), 0.20*e);
            end
        end

        % Overlay mask in magenta
        if S.showOverlay && any(Osl(:))
            ...
            if any(Eo(:))
                ...
                RGB(:,:,2) = max(RGB(:,:,2), 0.28*e);
                RGB(:,:,3) = max(RGB(:,:,3), 0.95*e);
            end
        end
